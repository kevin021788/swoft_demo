 ...
 * @Breaker("test")
 */
class TestBreaker extends CircuitBreaker
{
    /**
     * The number of successive failures
     * If the arrival, the state switch to open
     *
     * @Value(name="${config.breaker.test.failCount}", env="${USER_BREAKER_FAIL_COUNT}")
     * @var int
     */
     ...
     *
     * @Value(name="${config.breaker.test.successCount}", env="${USER_BREAKER_SUCCESS_COUNT}")
     * @var int
     */
     ...
     *
     * @Value(name="${config.breaker.test.delayTime}", env="${USER_BREAKER_DELAY_TIME}")
     * @var int
     */
