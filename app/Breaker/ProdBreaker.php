 ...
 * @Breaker("prod")
 */
class ProdBreaker extends CircuitBreaker
{
    /**
     * The number of successive failures
     * If the arrival, the state switch to open
     *
     * @Value(name="${config.breaker.prod.failCount}", env="${USER_BREAKER_FAIL_COUNT}")
     * @var int
     */
     ...
     *
     * @Value(name="${config.breaker.prod.successCount}", env="${USER_BREAKER_SUCCESS_COUNT}")
     * @var int
     */
     ...
     *
     * @Value(name="${config.breaker.prod.delayTime}", env="${USER_BREAKER_DELAY_TIME}")
     * @var int
     */
